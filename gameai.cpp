...
double Gameai::checkai(int aicards[]){
...
void Gameai::aibet(int &aimoney, int aicards[], int &bet){
  string aistatus = decision(aicards);
  int betamount = checkai(aicards) * 100;
  if ( aistatus == "GO"){
...
void Gameai::halfDealHand(int aicards[]){
  srand(time(NULL));
  int seed = rand();
  srand(seed);
  for (int i =0; i < 3; i++){
...
void Gameai::secondhalfDealHand(int aicards[]){
  srand(time(NULL));
  int seed = rand();
  srand(seed);
  for (int i =3; i < 5; i++){
